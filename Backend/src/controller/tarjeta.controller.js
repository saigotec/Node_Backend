...
const getTarjetas = async (req, res) => {
    try {
        ...
        const [rows] = await connection.query({sql: "SELECT id, agregarNuevaTarjeta, numeroTarjeta, fecha, anio, CCV FROM Tarjeta", rowsAsArray: false});
...
const getTarjeta = async (req, res) => {
    try {
        const { id } = req.params;
        const connection = await getConnection();
        const [rows] = await connection.query({sql: "SELECT id, agregarNuevaTarjeta, numeroTarjeta, fecha, anio, CCV FROM Tarjeta WHERE id = ?", values: [id], rowsAsArray: false});
...
const addTarjeta = async (req, res) => {
    try {
        const { agregarNuevaTarjeta, numeroTarjeta, fecha, anio, CCV } = req.body;

        if (agregarNuevaTarjeta === undefined || numeroTarjeta === undefined || fecha === undefined || anio === undefined || CCV === undefined) {
            res.status(400).json({ message: "Bad Request. Please fill all fields." });
            return;
        }

        const tarjeta = { agregarNuevaTarjeta, numeroTarjeta, fecha, anio, CCV };
        const connection = await getConnection();
